%% save_augMat_and_maskMat
% (06/28/2014)
%=========================================================================%
% - Save data augmentation (zero-padding) matrix and masking matrix
%   needed for the fft-based admm algorithm for fused lasso
%=========================================================================%
%%
clear all;
purge
...
outPath = [rootdir,'/augmat_maskmat_',parcellation,'.mat'];
outVars={'b', 'A', 'timeStamp',  'mFileName'};
%%
load([rootdir,'/graph_info_',parcellation,'.mat'],'adjmat','coord')
d=coord.nsamp;
p=d*(d-1)/2;
NSIZE=coord.NSIZE;
...
% w=randn(p,1);
% w=ones(p,1);
load([get_rootdir,'/data_local/designMatrix_FC_',parcellation,'.mat'],'X')
w=X(1,:)';
