function [W,output]=tak_EN_OCP2_regr_FISTA(X,Y,lam,gam,eta,options,F,wtrue)
% [W,output]=tak_EN_OCP2_regr_FISTA(X,Y,lam,gam,eta,options,F,wtrue)
%=========================================================================%
...
if(~exist('options','var')||isempty(options)),        
    ...
    tau = 1/(tnormest(X)^2+gam+eta*tnormest(F'*F));
else
    % step size (needs knowledge of spectral norm of hessian)
...
%=========================================================================%
GRAD = @(W) Xt*(X*W) - Xty + gam*W + eta*(W*FtF);
