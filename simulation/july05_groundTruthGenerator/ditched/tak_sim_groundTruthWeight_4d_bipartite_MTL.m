function [W,idx_supp,supp_info,C] = tak_sim_groundTruthWeight_4d_bipartite_MTL(nx,ny,q)
...
for ii=1:q
    Wtmp = zeros(p,1);
    Wtmp(idx_supp) = tak_sample_signed_unif([2,8],1) + 1*randn([length(idx_supp),1]);
    W(:,ii) = Wtmp(:);
end
%%
if nargout==4
