...
options.lambda=2^-23;  % L1 penalty weight
options.gamma =2^-25; % fused lasso penalty weight
...
%-------------------------------------------------------------------------%
% spectral norm: normest by far the fastest...
%-------------------------------------------------------------------------%
% tic
% s1=svds(C,1);
% toc
% tic
% [UU,SS,VV]=svd(full(C),'econ');
% toc
% abs(s1-max(diag(SS)))
% tic
% s2=normest(C)
...
% normest([speye(p);C])
% tic
options.tau=1/normest([speye(p);C]);
options.tau=options.tau/10;
% toc
% return
