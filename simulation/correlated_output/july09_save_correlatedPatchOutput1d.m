%% july09_save_correlatedPatchOutput1d.m.m
% (07/09/2014)
%=========================================================================%
% - "Patch" version of july09_save_correlatedSmoothOutput1d.m
% - however, i moved away from the "AR" based method of getting the
%   "dictionary weight coefficients"...now i just use a randomly signed
%   row vectors with additive noise
%-------------------------------------------------------------------------%
...
dict.natoms = 15; % <- # atoms per dictionary
dict.nsubset = 10; % <- # atoms per mixed signal (D *alpha, a
...
%         nPatches = randsample(3,1) + 2; % {3,...,5}
        pulseLenBase = round(p/1000)*(randsample(2,1)+2);
...
%     mixingMatrix = tak_sample_AR1d(sizeCluster, mixerCorr, dict.natoms);
    mixingMatrix = ones(dict.natoms,sizeCluster) .* repmat(sign(randn(1,sizeCluster)),[dict.natoms,1]) + .05*randn(dict.natoms,sizeCluster);
...
% save([rootdir,'/correlatedWeight_july09_1d_patch1.mat'],'W','idxCluster','dict','mFileName','timeStamp')
