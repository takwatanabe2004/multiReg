...
% enet-admm
%-------------------------------------------------------------------------%
[w_EN,out_EN]=tak_admm_EN_regr(X,y,lam,gam,options,wtrue);
%% compare
figure,imexp
subplot(241)
    tplot2(log10(out_GN_ADM.fval), log10(out_GN_IST.fval))
    legend('ADMM','ISTA'),title('log10(function value)')
subplot(242)
    tplot2(log10(out_GN_ADM.wdist),log10(out_GN_IST.wdist))
    legend('ADMM','ISTA'),title('log10(||wtrue-west||)')
subplot(243),
    tplot2(log10(out_GN_ADM.rel_changevec),log10(out_GN_IST.rel_changevec)), 
    legend('ADMM','ISTA'),title('log10(wnew-wold)')
subplot(244),tplot(wtrue)
subplot(245),tplot(w_GN_ADM)
subplot(246),tplot(w_GN_IST)
subplot(247),tplot3(w_GN_ADM,w_GN_IST,wtrue), 
    HH=legend('w_est_ADM','w_est_IST','wtrue');set(HH,'Interpreter','none')
subplot(248),
    tplot(abs(w_GN_ADM-w_GN_IST)),title('|w_ADMM-w_ISTA|','Interpreter','none')
% return
%% compare methods
% purge
figure,imexp
...
subplot(413),tstairs2(wtrue,w_GN_ADM),ylim(YLIM)
subplot(414),tstairs2(wtrue,w_GN_IST),ylim(YLIM)
drawnow
