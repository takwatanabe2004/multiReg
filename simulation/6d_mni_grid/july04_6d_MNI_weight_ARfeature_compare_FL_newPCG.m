%% july04_6d_MNI_weight_ARfeature_compare_FL_newPCG.m
% (07/04/2014)
%=========================================================================%
% I designed a "loopless" PCG algorithm...see how fast FL-FFT-ADMM with this.
% Compare with old "looping PCG" version....where loop is over the # outputs.
%=========================================================================%
%%
clear
purge
...
PCG.tol = 1e-6;
PCG.maxiter = 500;
PCG.precond = false; % use ichol-based preconditioner?
    
% FL-ADMM-MTL
options.MTL=true;

disp('--- FL-MTL (my loopless CG)---')
[w.FL_MTL_my_cg, output.FL_MTL_mypcg]=tak_FL_regr_MTL_ADMM_MY_PCG(X,Y,lam_FL,gam_FL,options,C,PCG,Wtrue);
Ypred.FL_MTL_my_cg = Xtest*w.FL_MTL_my_cg;
MSE.FL_MTL_my_cg =  norm(Ytest(:) - Ypred.FL_MTL_my_cg(:));
CORR.FL_MTL_my_cg = corr(Ytest(:),  Ypred.FL_MTL_my_cg(:));

disp('--- FL-MTL (my loopless PCG)---')
PCG.precond=true;
[w.FL_MTL_my_pcg, output.FL_MTL_mypcg]=tak_FL_regr_MTL_ADMM_MY_PCG(X,Y,lam_FL,gam_FL,options,C,PCG,Wtrue);
Ypred.FL_MTL_my_pcg = Xtest*w.FL_MTL_my_pcg;
MSE.FL_MTL_my_pcg =  norm(Ytest(:) - Ypred.FL_MTL_my_pcg(:));
CORR.FL_MTL_my_pcg = corr(Ytest(:),  Ypred.FL_MTL_my_pcg(:));


disp('--- FL-MTL (looping PCG)---')
